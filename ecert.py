...
mainSel = [
  inquirer.List('main',
                ...
                choices=['Sign', 'Verify', 'Encrypt', 'Decrypt', 'Certificate', 'Exit'],
            ),
]

def writeSignature(signature):
    sigFileName = datetime.now().strftime("%d%m%y_%H%M%S")
    with open(f"{sigFileName}.sig", 'wb') as f:
        f.write(signature)
...
def sign(data,p12Path):
    ...
    signature = base64.b64encode(OpenSSL.crypto.sign(pk,data,"sha256"))
...
def loadP12(p12Path):
    pwd = getpass.getpass(prompt = 'Password: ').encode()
    try:
